...
func.func @main() {
  ...
  %gx = arith.constant 100 : index
  %thx = arith.constant 50 : index
  gpu.launch blocks(%bx, %by, %bz) in (%grid_x = %gx, %grid_y = %c1, %grid_z = %c1)
             threads(%tx, %ty, %tz) in (%block_x = %thx, %block_y = %c1, %block_z = %c1) {
    %j0 = arith.muli %bx, %thx: index
